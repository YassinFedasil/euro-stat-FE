...
export default function BarChartOutRange({ onTop10Change }: BarChartOutRangeProps) {
    ...
    const fetchOutsRange = async (last: number) => {
        ...
        try {
            ...
            console.log("Données outs range reçues:", resData);
    ...
    useEffect(() => {
        fetchOutsRange(lastDraws);
    }, [lastDraws]);

    // Fonction pour extraire le nombre de tranches différentes dans une combinaison
    /*const getRangeCount = (label: string) => {
